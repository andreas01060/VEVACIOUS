...
for i in range(0,len(y)):
   for j in range(0,len(At)):
        if ((y[i]>0.3*x[j]-500 and y[i]<0.3*x[j]+1500)):
            ...
            os.chdir('../')
            mycmd='./Vevacious-1.2.03/Vevacious/bin/Vevacious.exe --input=./VevaciousInitializationTopTau.xml'
            os.system(mycmd)
	    exists=os.path.isfile('./TopTau.vout')
            if exists:            
                with open("./TopTau.vout","r") as file:
                    data=file.readlines()
                print(data[3])
...
with open('./resultsTopTau.txt', 'w+') as filehandle:
