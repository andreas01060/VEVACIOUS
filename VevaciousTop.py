...
extrater=1000/50
At=x[:]+1000/50
os.chdir('/afs/cern.ch/user/a/abally/VEVACIOUS/')
print(At)
for i in range(0,len(y)):
   for j in range(0,len(At)):
        if ((y[i]>0.3*x[j]-500 and y[i]<0.3*x[j]+1500)):
            ...
            os.chdir('../')
            mycmd='./Vevacious-1.2.03/Vevacious/bin/Vevacious.exe --input=./VevaciousInitializationTop.xml'
            os.system(mycmd)
	    exists=os.path.isfile('./Top.vout')
            if exists:            
                with open("./Top.vout","r") as file:
                    data=file.readlines()
                print(data[3])
...
with open('./resultsTop.txt', 'w+') as filehandle:
